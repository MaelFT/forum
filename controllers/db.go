...
func (r *SQLiteRepository) GetPostByID(id int64) (*models.Posts, error) {
	...
	like := 0
	is_Like := true
	isLiked, err := postIsLiked(db, int(posts.ID))
	if err != nil {
		panic(err)
	}
	if isLiked {
...
func userHasLikedPost(db *sql.DB, user_ID int, post_ID int) (bool, error) {
	query := "SELECT COUNT(*) FROM like WHERE user_id = ? AND post_id = ?"
	var count int
	err := db.QueryRow(query, user_ID, post_ID).Scan(&count)
...
func userHasDisLikedPost(db *sql.DB, user_ID int, post_ID int) (bool, error) {
	query := "SELECT value FROM like WHERE user_id = ? AND post_id = ?"
	var count int
	err := db.QueryRow(query, user_ID, post_ID).Scan(&count)
...
func deleteLikesByUser(db *sql.DB, userID int) error {
	// requête SQL pour supprimer les entrées dans la table "likes" correspondant à l'utilisateur spécifié
	query := "DELETE FROM like WHERE user_id = ?"

	// exécuter la requête avec le paramètre userID
...
func postIsLiked(db *sql.DB, postID int) (bool, error) {
